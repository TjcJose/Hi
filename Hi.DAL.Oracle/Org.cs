...
namespace Hi.DAL.Oracle
{
    ...
    public class OrgDAL:IOrg
    {
        ...
        private string DepartmentSql(Dictionary<string, string> dict)
        {
            ...
            strb.Append("select t.*," + Hi.Common.Db.DbUtils.GetPageRowNum("id") + " from bas_org t");
        ...
        {
            string strSql = "select * from bas_org_online where  org_id = " + id.ToString();
            Org model = new Org();
            DataRow dr = DataBase.ExecuteDataRow(strSql);
            if (dr == null)
                return null;
            model.Id = int.Parse(dr["org_id"].ToString());
