...
namespace Hi.DAL.MsSql
{
    ...
    public class User : IUser
    {
        ...
        {
            string strSql = "select * from bas_user where  usercode = '" + user_code + "'";
            DataRow dr = DataBase.ExecuteDataRow(strSql);
            return dr;
        }
        private DataRow DetailFromId(string id)
        {
            string strSql = "select * from bas_user where  userid = " + id + "";
            DataRow dr = DataBase.ExecuteDataRow(strSql);
            return dr;
        }
        public BasUser Detail(string id)
        {
            ...
            model.UserId = dr["userid"].ToString().Trim();
            model.OrgId = dr["orgid"].ToString().Trim();
            model.OrgName = dr["orgname"].ToString().Trim();
            model.UserCode = dr["usercode"].ToString().Trim();
            model.UserPassword = dr["userpassword"].ToString().Trim();
            model.RealName = dr["realname"].ToString().Trim();
            model.RoleId = dr["roleid"].ToString().Trim();
        ...
        private string GetSql(Dictionary<string, string> dict)
        {
            ...
            strb.Append(" select t.*,(case when sex='1' then '男' else '女' end ) as sex_title ");
